...
- `timeline_math.py`: stdlib-only numeric runtime-planning boundary for full-output/Preview windows, half-open interval clipping/rebasing/coverage, time/frame quantization, canonical ending/Loop/fade bounds, codec Loop-tail protection, and pure Style eligibility. Import direction is `app.py`/`renderer.py`/`datamosh.py` -> `timeline_math.py`, never back toward UI, media, codec, probing, state, or filesystem ownership.
- `coverage_intervals.py`: stdlib-only, DTO-free interval-policy boundary shared by ANSI Coverage and Style FX Coverage after caller-owned mode/time normalization. It owns numeric manual merge/clip, local seeded-Random gap placement, percentage/min/max handling, and the final immutable interval tuple. Import direction is `renderer.py` -> `coverage_intervals.py` -> `timeline_math.py` -> stdlib; it must not import UI, state, renderer, media/probe, codec, audio, or filesystem ownership.
- `zone_motion.py`: stdlib-only deterministic Zone geometry boundary. It consumes normalized `ZoneDefinition` values and resolves Drift/Pulse rectangles from the Weirdness seed, stable Zone ID, absolute assembled-output time, and full-output duration. Import direction is `renderer.py` -> `zone_motion.py` -> `state_contract.py` -> stdlib; it must not import Qt, renderer/media/probe/codec modules, NumPy/OpenCV/Pillow, paths, or filesystem ownership.
- `renderer.py`: desktop render pipeline, media-derived full-output/Preview planning, timeline expansion, Random assembly, frame rendering, ANSI/chunky conversion, effects, transitions, codec/audio coordination, explicit saved codec Layer dispatch, and optimization handoff. It consumes `coverage_intervals.py` for caller-neutral ANSI/Style FX numeric interval construction and `timeline_math.py` for generic window/interval/frame decisions.
...
- `timeline_math.py`: affects full/Preview window lengths, interval rebasing, frame boundaries, ending/Loop/fade placement, codec tail protection, and Style eligibility. Keep it stdlib-only and numeric; edits require `tests.test_timeline_math_contract`, the exact Material/planning oracles, and the full desktop suite.
- `coverage_intervals.py`: affects both ANSI normal-video intervals and Style FX clean/protected intervals, including seeded placement and exact gap/min/max behavior. Keep it caller-neutral, stdlib-only except for `timeline_math.py`, and DTO/media/UI/filesystem-free; edits require `tests.test_coverage_intervals_contract`, focused Preview/codec-protection tests, exact unchanged Material/planning oracles, and the full desktop suite twice plus reverse order.
- `zone_motion.py`: affects deterministic Drift/Pulse containment, Preview/full-output geometry parity, seed behavior, Loop closure, and the static SKRRT boundary. Keep it stdlib-only except for `state_contract.py`; edits require `tests.test_zone_motion_contract`, all-five Material containment, moving Circuit history, Preview-window geometry, unchanged static Full Frame/Zone/SKRRT evidence, performance comparison, and the full desktop suite.
