...
- `tests/`: tracked, rights-safe, deterministic desktop regression contracts for the pure timeline-math boundary, Full Frame Material oracle, Material seed behavior, frame-effect Zones, schema-6 migration/repair, isolated settings state, the canonical full-output/Preview planning oracle, controlled MPEG-4 structure/modes, Layer ordering, frame transport/failure classification, final H.264/AAC identity, source immutability, and temporary cleanup. It generates all frame/media fixtures at runtime in temporary directories, uses the Python standard-library `unittest` runner plus existing runtime dependencies/tools, and must not contain private media or generated evidence.
...
```bash
python3 -m py_compile app.py state_contract.py timeline_math.py app_i18n.py renderer.py datamosh.py ffmpeg_utils.py still_cache.py presets.py theme.py run.py scripts/generate_logo.py scripts/generate_icon.py scripts/generate_ui_textures.py scripts/generate_branding.py
