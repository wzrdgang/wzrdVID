...
- `tests/`: tracked, rights-safe, deterministic desktop regression contracts for the Full Frame Material oracle, Material seed behavior, frame-effect Zones, schema-6 migration/repair, isolated settings state, the canonical full-output/Preview planning oracle, controlled MPEG-4 structure/modes, Layer ordering, frame transport/failure classification, final H.264/AAC identity, source immutability, and temporary cleanup. It generates all frame/media fixtures at runtime in temporary directories, uses the Python standard-library `unittest` runner plus existing runtime dependencies/tools, and must not contain private media or generated evidence.
