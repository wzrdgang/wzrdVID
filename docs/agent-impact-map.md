...
### Authentic DATAMOSHING Codec Stage

- Owning files/directories: `datamosh.py`, with a narrow call site in `renderer.py` and the default-off UI/settings flag in `app.py`/`app_i18n.py`.
- Purpose: When DATAMOSHING is enabled, re-encodes the completed silent visual intermediate as a controlled video-only MPEG-4 Part 2 elementary `.m4v` stream, linearly parses `00 00 01 B6` VOP units, deterministically suppresses selected later I resets by substituting encoded P VOPs, repeats selected encoded P VOPs for prediction persistence, and transcodes the manipulated stream back to a validated silent H.264/yuv420p MP4 before any existing audio path runs.
- Inbound dependencies: `renderer.py` silent H.264 intermediate, output fps/frame count, Effect Intensity, Weirdness seed, absolute preview/output offset, `Style begins at`, Loop Friendly, and existing CRF/target-bitrate planning.
- Outbound dependencies: existing `ffmpeg_utils.py` binary discovery/command runner, system ffmpeg native `mpeg4` encoder and ffprobe, a nested `datamosh/controlled_prediction.m4v`, `datamosh/manipulated_prediction.m4v`, and `datamoshed_silent.mp4` inside the app-owned `wzrd_vid_render_*` temporary directory. No persistent cache or user source is written.
- Error boundary: `DatamoshError` identifies intermediate encode, invalid/truncated/no-VOP structure, manipulated-stream decode/transcode, or safe-output validation failures. These errors propagate after frame rendering; they must not trigger PNG fallback, silently disable DATAMOSHING, substitute Motion Melt, or claim success. The outer render temporary directory cleans success and failure artifacts.
- Boundary/loop behavior: the pre-style prefix is protected, the first styled frame is forced to a clean I anchor, deterministic events are eligible only after it, preview decisions use absolute output frame positions, and Loop Friendly conservatively protects the final codec GOP. Very short eligible suffixes can log that no event was available and keep the already-rendered silent intermediate.
- Required validation: parse rejection; I/P-only controlled map; both I-reset suppression and P persistence; same-seed bytes/event map plus changed-seed map; low/medium/high intensity; exact decoded frame count and duration within one frame; Style/preview/loop boundaries; source hashes and temp cleanup; DATAMOSHING-specific failure injection with no PNG fallback; Phase 2 interactions; default/forced/fallback transports; H.264/yuv420p and AAC/audio timing; and fresh PyInstaller bundle import/render.
...
- Data/state path: `RenderSettings`, `PlaybackPlan`, optional `style_begin_time`, preview-only `output_time_offset`, optional `max_video_length`, optional `random_clip_assembly`, bypass intervals, random seeds, style/output/optimization/framing/effect settings. The generic schema-version-5 `effects` dictionary persists DATAMOSHING plus the five v0.3.0 frame-effect flags without a schema bump; recipes that lack those keys load them off. Blank or `auto` max video length means no cap/full selected timeline; random clip assembly uses that selected timeline duration unless an explicit max length is provided.
- Asset/media path: source media, temp frame directories and optional nested DATAMOSHING elementary streams, preview folder under app settings parent, user-selected output path, optional optimized output path.
- Success behavior: Normal MP4 visually renders ANSI/text-art/glitch output, optional `Style begins at` keeps output frames before its non-negative timestamp clean and applies the full existing visual treatment at/after the boundary, optional max-length caps output duration, optional random clip assembly builds deterministic seeded source segments before rendering, optional audio is muxed/mixed independently of the visual gate, optional optimization targets max MB, output/open-folder controls become available. DATAMOSHING, when enabled, runs on the completed silent visual output after all frame-domain effects/transitions/endings and before unchanged audio handling; when off it adds no codec round trip. Preview rendering compares the gate against the preview window's absolute output time. `0:00` preserves all-styled behavior; a gate at/after the output end keeps all rendered frames clean. Blank/auto Max Video Length stays blank in saved settings and support/render reports unless the user explicitly types a cap; old no-op saved values that match or exceed the selected timeline normalize back to auto. By default the silent MP4 streams rendered RGB frames directly to ffmpeg through the frame-pipe transport. Legacy PNG frame staging can be forced by local desktop developer setting or `WZRDVID_FORCE_PNG_STAGING=1`, and the renderer falls back to PNG staging only on pre-audio pipe/encode transport failure. Preview MP4s and still/HEIC proxies under app-owned cache folders can be cleared manually, and old app-managed preview/cache/temp files are cleaned best-effort at launch.
...
- Data/state path: user-selected recipe JSON files; app settings store recent/default values. Schema version 5 adds `style_begin_time` as entered text; recipes/settings without the field load it as `0:00`. Schema version 4 added `max_video_length` as entered text and `random_clip_assembly` as a boolean; those missing fields still load as blank/false.
- Asset/media path: recipe JSON references user media paths but does not copy media.
- Success behavior: Timeline, audio, Style begins at, max-length/random assembly controls, style/effects, framing, bypass, output, batch, seeds, and optimization controls restore.
...
| `datamosh.py` | Encoded MPEG-4 Part 2 prediction manipulation and safe pre-audio transcode | DATAMOSHING preview/full/batch outputs, timing, temp cleanup, final codec validity | Keep parsing bounded and prediction-aware; never mutate sources or silently simulate/fallback | `py_compile`; VOP/authenticity/determinism/failure tests; ffprobe frame/duration/H.264/AAC checks |
...
| ffmpeg / ffprobe | Media probing, encode/mux/mix/optimize and controlled MPEG-4 Part 2 DATAMOSHING round trip | `ffmpeg_utils.py`, `datamosh.py`, README/docs | External install required; native `mpeg4`, `libx264`, I/P/VOP structure, frame count, and path detection must be validated |
...
| App logic/state | `python3 -m py_compile app.py app_i18n.py renderer.py datamosh.py ffmpeg_utils.py still_cache.py presets.py theme.py run.py`; focused render/preview/project smoke; check settings backward compatibility |
| Video/media handling | `python3 -m py_compile app.py app_i18n.py renderer.py datamosh.py ffmpeg_utils.py still_cache.py presets.py theme.py run.py`; tiny render or still/HEIC cache smoke with relevant media; authentic VOP and failure-boundary tests if DATAMOSHING changed; `node --check docs/i18n.js docs/lite/app.js` if Lite/media UI changed; ffprobe final codec/audio where output changed |
