...
- High-risk notes: the accepted serialized Material oracle SHA-256 is `441f9150b0f8c2d79fadb5a653a4b930d777959c37e458099a3b28eee3baa80a`; the accepted canonical full-output/Preview planning oracle is exactly 31,416 UTF-8 bytes with SHA-256 `85477bbf54769f2fdabd2ffe05ac3b7fa34017216068c80beb6d107a502e0dc3`; canonical spILL! Low/Medium/High changed/recursive/max-depth counts are `10/0/0`, `42/16/4`, and `60/60/7`. Do not regenerate or weaken accepted behavior to accommodate a production change. Expensive auxiliary stress, long-form/performance, frozen/package, DMG, subjective/real-media, and quarantine validation remain qualification-only.

### Pure Timeline Math Boundary

- Owning file: `timeline_math.py`, with direct numeric contracts in `tests/test_timeline_math_contract.py` and narrow consumers in `app.py`, `renderer.py`, and `datamosh.py`.
- Purpose: one stdlib-only source of truth for full-output/Preview duration and local/absolute time conversion, half-open interval intersection/rebasing/merging/coverage, time-to-frame and local-to-absolute frame quantization, canonical ending/visual-Loop/fade intervals, codec Loop-tail protection, and pure Style eligible-frame conversion.
- Inbound dependencies: plain numbers and interval tuples from Preview slicing, renderer orchestration, and codec-tail compatibility wrappers.
- Outbound dependencies: Python `math` and `collections.abc` only. Import direction is `app.py`/`renderer.py`/`datamosh.py` -> `timeline_math.py`; the pure module must never import Qt, NumPy, Pillow/OpenCV, render/codec DTOs, probing/media helpers, persisted state, audio/filesystem execution, or its callers.
- High-risk notes: changes can shift exact half-open boundaries, Preview-local clocks, frame eligibility, and true-tail behavior. Run the direct pure-module tests, inspect the stdlib-only import graph, retain the exact Material and 31,416-byte planning oracles, and run the full desktop suite for every edit.

### Render Engine

- Owning files/directories: `renderer.py`, `datamosh.py`, `presets.py`, consuming pure numeric decisions from `timeline_math.py`.
...
- Owning files/directories: `app.py`/`app_i18n.py` for controls, persistence, and user-facing copy; `renderer.py` for interval construction, source-stage frame gating, and temporal resets; `timeline_math.py` for Preview interval rebasing and frame-range math; `datamosh.py` for codec protection.
...
| App logic/state | `python3 -m py_compile app.py state_contract.py timeline_math.py app_i18n.py renderer.py datamosh.py ffmpeg_utils.py still_cache.py presets.py theme.py run.py`; direct pure state-module tests plus isolated MainWindow integration; tracked desktop regression suite when schema, Zones, Material state, Preview/playback/transition/tail planning, codec execution/Layer persistence, render transport/failure classification, or final audio/media handling are affected; direct pure timeline-math tests and import inspection when that boundary changes; focused render/preview/project smoke; check settings backward compatibility |
| Video/media handling | `python3 -m py_compile app.py state_contract.py timeline_math.py app_i18n.py renderer.py datamosh.py ffmpeg_utils.py still_cache.py presets.py theme.py run.py`; tracked desktop regression suite for affected Preview/playback/transition/tail, frame, Zone, codec, Layer, transport/failure, or final-media contracts; direct pure timeline-math tests and import inspection when that boundary changes; tiny render or still/HEIC cache smoke with relevant media; `node --check docs/i18n.js docs/lite/app.js` if Lite/media UI changed; ffprobe final codec/audio where output changed |
| Tracked desktop regression tests | Run `PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m unittest discover -s tests -p 'test_*.py'`; require exact Material SHA-256 `441f9150b0f8c2d79fadb5a653a4b930d777959c37e458099a3b28eee3baa80a`, exact 31,416-byte planning SHA-256 `85477bbf54769f2fdabd2ffe05ac3b7fa34017216068c80beb6d107a502e0dc3`, canonical spILL! contracts, direct case-level full-output/Preview diagnostics, rights-safe deterministic generated fixtures/media, isolated settings/temp cleanup, source immutability, no repository artifacts, repeat execution, and reverse-module-order execution when the suite or its fixtures/oracles change |
