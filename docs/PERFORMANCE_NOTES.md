...
- Test media: 8 real user HEIC files copied to `/tmp/wzrdvid-lite-real-heic-profile/heic-batch`, about 6.24 MB total. The first sampled files were 4032x3024 phone stills. A generated local 20s WAV tone was used for the Safari desktop added-audio check; the WKWebView harness generated its own short WAV.
- Safari desktop profile: local Lite was served from `http://127.0.0.1:8770/lite/` and manually tested in Safari because Safari WebDriver and Apple Events JavaScript automation were disabled in Safari settings. Importing the 8-file HEIC batch loaded 8/8 files with 0 failures and logged `import profile: 8 timeline file(s), 8 loaded, 0 failed, 8 HEIC/HEIF, 0.05s`; per-file HEIC decode logs showed mostly `0.00s` to `0.01s`. A 30s random clip render with added audio completed and produced an MP4 download-ready state. Clear Project removed selected media, added audio, rendered output state, and allowed selecting the same HEIC batch again while keeping durable UI preferences.
- Safari memory observation: total Safari/WebKit RSS was about 596 MB after import and about 1.05 GB during/after the 30s render, with the largest WebContent process around 348 MB and WebKit GPU around 263 MB. This did not show obvious runaway memory pressure for the 8-file batch, though it is not a substitute for broader device profiling.
- Apple Lite WKWebView profile: a temporary app copy under `/tmp/wzrdvid-apple-heic-profile` embedded the same 8 HEIC payloads into the debug smoke harness so no tracked Apple Lite files changed. On the iPhone 17 simulator, WKWebView imported 8/8 HEIC files with 0 failures and logged `import profile: 8 timeline file(s), 8 loaded, 0 failed, 8 HEIC/HEIF, 0.66s`; per-file HEIC decode was about 0.07s to 0.13s. Render start delay was 86 ms, the 15s random HEIC render completed in 15.19s, output diagnostics showed MP4/H.264 with one video track, one audio track through Web Audio, 451 frames for a 450-frame target, 8 timeline sources, and a 12.19 MB blob. Clear Project reset runtime state and the same HEIC batch reselected successfully after reset.
- Proxy decision: do not add a Lite still-proxy cache now. Both Safari and WKWebView decoded the real 8-file HEIC batch quickly, render start was not blocked by still decode, and Clear Project released runtime state as intended. A memory-only per-session ImageBitmap/canvas proxy remains a future option only if larger real batches show repeated decode/downscale cost or memory churn. Do not add IndexedDB, CacheStorage, service workers, localStorage media blobs, uploads, or persistent media storage for this.
- Apple Lite implication: no separate native bridge/cache is needed for HEIC import/reset based on this profile. Apple Lite should keep inheriting the browser Lite runtime, with the native shell limited to local WKWebView loading and rendered-clip export/save behavior.
- Known gaps: Safari desktop was manually observed because automation permissions were unavailable. iOS Safari on a real device was not profiled in this pass. The WKWebView profile used the simulator, not the connected real iPhone/iPad. Larger batches and older devices may still justify a future memory-only proxy experiment.
...
## 2026-05-12 - Lite/Apple portability audit for HEIC cache and frame transport

- Goal: decide whether the recent desktop HEIC/photo cache work and default ffmpeg frame-pipe transport should be integrated into `docs/lite`, `wzrdvid.com/lite`, Apple Lite, or a native Apple app path.
- Desktop status: the desktop app now accepts HEIC/HEIF through the normal media picker, defers HEIC decode until preview/render, caches app-managed HEIC proxy PNGs under `StillCache`, reuses warm proxies during planning/render, keeps the subtle HEIC motion loop, and uses direct raw RGB ffmpeg frame piping by default with PNG staging as local opt-out/fallback.
- Web Lite status: `docs/lite/index.html` and `docs/lite/app.js` already accept `.heic`/`.heif` by extension and `image/*`, but decode remains browser-dependent through `Image.decode()`. Lite already avoids file-based frame staging: frames are drawn directly into Canvas and captured through `canvas.captureStream()` plus MediaRecorder into an in-memory Blob/object URL.
- Apple Lite status: Apple Lite bundles the same `docs/lite` runtime in WKWebView, so it inherits the browser-side HEIC acceptance limits and Canvas/MediaRecorder transport. Its native code only handles local wrapper concerns: loading bundled assets, validating the rendered MP4 payload, and saving to Photos. It should not import the desktop ffmpeg frame pipe or desktop still-cache code.
- Full native Apple app status: no separate native renderer path is present in this repo beyond the Apple Lite WKWebView shell. Any native HEIC proxy cache or AVFoundation/Metal render transport would be new design work, not a safe transplant.
- Integration decision: no source behavior changed in this audit. The portable idea is the *principle* of early downscale/proxy reuse for stills, but web Lite needs an explicit browser design such as per-session offscreen-canvas/image-bitmap proxies with quality validation, and Apple native would need an AVFoundation/Photos/Core Image cache design. The desktop frame pipe is not portable code; the platform equivalents are Canvas/MediaRecorder for web Lite and, later, AVAssetWriter/PixelBuffer-style streaming for a true native renderer.
- Recommended future work: prototype a per-session Lite still proxy only after measuring large-image/HEIC import on Safari/WKWebView. Keep it memory-only first, avoid IndexedDB/CacheStorage unless the privacy/storage copy is updated, and reject the change if it visibly diverges from current public Lite quality.
