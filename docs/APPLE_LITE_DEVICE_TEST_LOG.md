...
Date: 2026-05-10
Status: prior real-device import/added-audio/save/visual pass; Phase 5 source audio automated, physical-device source-audio hand test pending
...
## 2026-08-25 Phase 5 Lite Source Audio

- Added the localized, default-on global `Include Source Audio` control. Its state is page-local, survives Clear Project like equivalent render options, and is snapshotted when MAKE CLIP starts.
- Source video audio follows the same final normal/random segment and source-time map as the visual renderer. Per-source gains use an 18 ms anti-click ramp; still/image sections contribute source-bus silence.
- Source-only and mixed modes use one page-session AudioContext and one MediaStreamDestination track. Timeline media-element sources retain safe cached ownership across renders. Add-only preserves the existing captureStream-first/Web Audio fallback; the new mixed mode uses a decoded Add Audio buffer because iOS otherwise stops the Add media element when a source video begins.
- The iPhone 17 simulator smoke passed Source OFF/no Add, Source ON/no Add, Source OFF/Add, Source ON/Add, a 15-second random Fast render, a 30-second repeated Better render, 60-second control-state coverage, reset during an active render, one-context/no-active-connection cleanup, and native validation of one video plus one audio track.
- `/tmp/wzrdvid-phase5-lite-source-audio/simulator-audio-analysis.json` records exported-file PCM/FFT proof: Add-only includes 220 Hz and excludes 440/880 Hz; source-only includes 440 Hz and excludes 220/880 Hz; mixed representative windows contain 220 Hz plus the mapped 440/880 Hz source tone, still windows exclude both source tones, and post-cut windows exclude the prior source tone.
- No physical iPhone was available/authorized for a new hand-listen pass. Confirming source ON, source OFF, mixed Add Audio, render monitoring, and saved-Photos playback with real device media remains a manual release gate.
