...
- Required checks: `python3 -m py_compile app.py app_i18n.py renderer.py datamosh.py ffmpeg_utils.py still_cache.py presets.py theme.py run.py`; run `PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m unittest discover -s tests -p 'test_*.py'` when frame-domain Material or Zone behavior is affected; tiny render, still/HEIC cache, or Lite render smoke as relevant; authentic VOP/frame-count/duration/error-boundary tests when DATAMOSHING changes; for codec Layer changes, compare meaningful overlapping permutations while holding each mode's planned targets/parameters and SKRRT/Scatter auxiliary bytes fixed, verify final-writer attribution/output changes, and repeat source plus frozen-package renders; for SKRRT Zone changes, preserve the byte-exact Full Frame oracle, prove exact prepared inside/outside provenance, measure decoded leakage/recovery across representative geometry, retain strict one-I/P-only auxiliaries and one-main-encode/one-safe-transcode counts, and repeat transport/audio/failure/Layer cases from frozen modules; `node --check docs/i18n.js docs/lite/app.js` if Lite/media UI or copy changed; `ffprobe` final output if codec/audio changed; for Lite audio changes validate source-off/source-only/Add-only/mixed exported PCM against the visual segment map, still silence, cut leakage, repeated renders, and reset cleanup; grep network APIs for Lite privacy changes.
