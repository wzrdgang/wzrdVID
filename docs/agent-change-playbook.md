...
- Inspect first: `state_contract.py`, `zone_motion.py`, `app.py`, `app_i18n.py`, `renderer.py`, `datamosh.py`, `ffmpeg_utils.py`, `still_cache.py`, project save/load code, settings defaults and migration behavior.
- Searches to run: affected setting key, dataclass field, UI control name, project JSON serializer/deserializer.
- Common mistakes to avoid: changing settings schema without backward handling, duplicating effect activation state in ordering or Zone controls, allowing unknown/duplicate/missing codec Layer identifiers, silently resetting explicit Layer order from presets, making ShShSHa or another unapproved codec mode Zone-eligible, blocking the UI thread, losing paths with spaces, or breaking audio/render sync.
- High-risk files: `state_contract.py`, `zone_motion.py`, `app.py`, `app_i18n.py`, `renderer.py`, `datamosh.py`, `ffmpeg_utils.py`, `still_cache.py`.
- Required checks: `python3 -m py_compile app.py state_contract.py zone_motion.py app_i18n.py renderer.py datamosh.py ffmpeg_utils.py still_cache.py presets.py theme.py run.py`; run `PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m unittest discover -s tests -p 'test_*.py'` when schema, Zones, Material state, Preview/playback/transition/tail planning, codec execution/Layer persistence, render transport/failure classification, or final audio/media handling are affected; focused smoke test for affected flow. Persisted-state changes require direct pure-module schema-3/4/5/6, malformed/default/Reset/canonicalization/idempotence tests and an isolated offscreen MainWindow settings or recipe integration check. Keep `state_contract.py` stdlib-only with imports flowing from `app.py`/`renderer.py` into it, never back toward Qt, rendering, codec execution, probing, or filesystem ownership. Keep `zone_motion.py` stdlib-only except for `state_contract.py`; its changes require direct deterministic seed/containment/absolute-Preview/closure tests, moving Circuit history, unchanged static SKRRT evidence, source/frozen renders, and three-run 960x540 plus 1280x720 static-versus-moving performance comparisons. Persistent codec Layer changes also require exact save/reload/reset behavior, schema-3/4/5 missing/malformed migration, toggle/preset/Weird-order preservation, and duplicate/unknown execution rejection. Codec Zone eligibility changes additionally require exact assignment-row allowlisting, activation independence, legacy-schema/Full Frame migration, and source plus frozen-package checks.
...
- Common mistakes to avoid: audio desync, unsupported image orientation, temp file leaks, final codec regression, path quoting bugs, treating a DATAMOSHING codec failure as a frame-pipe failure, making SKRRT/Scatter auxiliary preparation consume an ordered mutated stream, broadening a SKRRT Zone to crop-only encoding or alpha compositing, changing Full Frame SKRRT evidence/bytes, or changing per-mode event/target/intensity plans merely because codec Layer order changed.
- High-risk files: `renderer.py`, `datamosh.py`, `ffmpeg_utils.py`, `still_cache.py`, `app.py`, `docs/lite/app.js`, `docs/i18n.js`.
- Required checks: `python3 -m py_compile app.py state_contract.py zone_motion.py app_i18n.py renderer.py datamosh.py ffmpeg_utils.py still_cache.py presets.py theme.py run.py`; run `PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m unittest discover -s tests -p 'test_*.py'` for affected Preview/playback/transition/tail, frame-domain Material, Zone, codec, Layer, render transport/failure, or final-media behavior; tiny render, still/HEIC cache, or Lite render smoke as relevant. Zone Motion changes require exact absolute-output-time geometry across Preview windows, deterministic seed/amount/cycle/closure tests, all-five containment and overlap, stable-ID bounded Circuit history, static SKRRT, performance comparisons, and source plus frozen-package renders. Codec Layer changes still require meaningful overlapping permutations with frozen per-mode plans/auxiliary bytes, final-writer attribution/output changes, and source plus frozen-package renders. SKRRT Zone changes still require the Full Frame oracle, exact prepared provenance, decoded leakage/recovery across representative geometry, strict auxiliaries/encode counts, and frozen transport/audio/failure/Layer cases. `node --check docs/i18n.js docs/lite/app.js` if Lite/media UI or copy changed; `ffprobe` final output if codec/audio changed; for Lite audio changes validate source-off/source-only/Add-only/mixed exported PCM against the visual segment map, still silence, cut leakage, repeated renders, and reset cleanup; grep network APIs for Lite privacy changes.
